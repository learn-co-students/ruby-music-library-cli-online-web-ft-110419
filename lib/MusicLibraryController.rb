class MusicLibraryController
  ...
  def initialize(path = './db/mp3s')
    ...
    #MusicImporter.import(new_importer_obj)
  end 
  ...
  def list_artists 
    Artist.all.sort { |a, b| a.name <=> b.name}.each.with_index(1) do |artist, idx|
      puts "#{idx}. #{artist.name}"
    end 
  end 
  
    def list_genres  
    Genre.all.sort { |a, b| a.name <=> b.name}.each.with_index(1) do |genre, idx|
      puts "#{idx}. #{genre.name}"
    end 
  end 
